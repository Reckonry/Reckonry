...
public sealed class BinanceReconciliationEngineTests
{
    ...
    {
        var root = Directory.CreateTempSubdirectory("ledgerforge-reconciliation-");
        try
        {
            var officialReports = Directory.CreateDirectory(Path.Combine(root.FullName, "official"));
            var ledgerReports = Directory.CreateDirectory(Path.Combine(root.FullName, "ledgerforge"));
