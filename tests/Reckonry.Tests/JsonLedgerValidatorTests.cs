...
public sealed class JsonLedgerValidatorTests
{
    ...
    {
        var outputFolder = Directory.CreateTempSubdirectory("reckonry-validator-");
        try
        {
    ...
    public void ValidateJson_ReturnsErrorsForLegacyArray()
    {
        ...
        Assert.Contains(result.Errors, e => e.Contains("Reckonry ledger v1 wrapper", StringComparison.Ordinal));
    }
    ...
    {
        var outputFolder = Directory.CreateTempSubdirectory("reckonry-legacy-ledger-");
        try
        {
