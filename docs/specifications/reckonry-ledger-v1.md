...
The Reckonry canonical ledger format is the durable interchange format between importers, reports, reconciliation, audit checks, and future tax modules.
...
```json
{
  "schemaVersion": "reckonry-ledger-v1",
  "metadata": {
...
- `amount`: positive JSON number. Reckonry uses `decimal` internally and never `double`.
- `direction`: `In` or `Out`.
...
The v1 schema version is `reckonry-ledger-v1`.
