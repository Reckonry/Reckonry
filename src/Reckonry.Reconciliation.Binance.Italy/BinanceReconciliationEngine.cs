...
public sealed partial class BinanceReconciliationEngine(
    ...
    : IBinanceReconciliationEngine, IReconciliationModule
{
    ...
    public ReconciliationModuleDescriptor Descriptor { get; } = new(
        "binance-italy",
        "Binance Italy Reconciliation",
        ReconciliationScope.ProviderCountry,
        ProviderId: "binance",
        CountryCode: "IT",
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(reckonryReportsFolder);
        ArgumentException.ThrowIfNullOrWhiteSpace(outputFolder);

        var documents = _reportReader.ReadFolder(officialReportsFolder);
        var snapshotYears = DetectReportYears(reckonryReportsFolder, "rw-snapshot-*.json");
        var valueYears = DetectReportYears(reckonryReportsFolder, "rw-value-*.json");
    ...
    async Task<ReconciliationRunResult> IReconciliationModule.ReconcileAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var summary = await ReconcileAsync(
            request.OfficialReportsFolder,
            request.ReckonryReportsFolder,
        ...
        IReadOnlyCollection<int> valueYears)
    {
        ...
        var status = metadata switch
        {
            ...
            _ when !snapshotYears.Contains(metadata.TaxYear!.Value) || !valueYears.Contains(metadata.TaxYear!.Value)
                => ReconciliationStatus.MissingReckonryReports,
