...
var runner = new LedgerForgeBenchmarkRunner(options);
    ...
    IReadOnlyList<int> Counts)
{
    public static BenchmarkOptions Parse(IReadOnlyList<string> args)
    {
        var outputPath = GetOption(args, "--out")
            ?? Path.Combine("benchmarks", "results", $"ledgerforge-benchmark-{DateTimeOffset.UtcNow:yyyyMMdd-HHmmss}.md");
...
internal sealed class LedgerForgeBenchmarkRunner
{
    ...
    public LedgerForgeBenchmarkRunner(BenchmarkOptions options)
    {
    ...
    {
        var scenarioFolder = Path.Combine(Path.GetTempPath(), "ledgerforge-benchmarks", Guid.NewGuid().ToString("N"));
    ...
    private static string BuildMarkdown(IReadOnlyList<ScenarioResult> results)
    {
        ...
        builder.AppendLine("# LedgerForge Benchmark Report");
        builder.AppendLine();
